...
def retrieve_rule_cards(query, top_k=3):
    ...
    # 핵심 카드와 점수 차이가 너무 큰 보조 카드는 제거한다.
    # 예: 121점 vs 16점이면 16점 카드는 주제 혼선을 만들 수 있으므로 제외.
    filtered = []
    for score, card in scored:
        ...
        if score == best_score:
            keep = True
        elif score >= best_score * 0.55:
            keep = True
        elif score >= 70:
            # 고속도로 사례처럼 관련 카드들이 모두 높은 점수인 경우 유지
            keep = True
    ...
    results = []
    for score, card in filtered[:top_k]:
        item = card.copy()
        item["score"] = score
        results.append(item)
...
def category_weight_for_query(query, category, source_file=""):
    q = query

    # 기본값
    weight = 1.0

    # PM / 전동킥보드 사고는 PM 과실자료가 가장 직접적
    if "전동킥보드" in q or "킥보드" in q or "PM" in q or "개인형" in q:
        if category == "fault_pm":
            weight *= 1.60
        elif category == "law":
            weight *= 1.15
        elif category == "easylaw_traffic":
            weight *= 1.10

    # 자전거횡단도는 PM 과실자료와 도로교통법 둘 다 의미 있음
    if "자전거횡단도" in q:
        if category == "fault_pm":
            weight *= 1.35
        elif category == "law":
            weight *= 1.30
        elif category == "easylaw_bicycle":
            weight *= 1.15

    # 우회전 + 보행자는 도로교통법/생활법령 교통운전 우선
    if "우회전" in q and "보행자" in q:
        if category == "law":
            weight *= 2.00
        elif category == "easylaw_traffic":
            weight *= 1.80
        elif category == "fault_pm":
            weight *= 0.45
        elif category == "easylaw_bicycle":
            weight *= 0.65

    # 신호 없는 교차로는 도로교통법/생활법령 교통운전 우선
    if "신호등 없는" in q or "신호 없는" in q or "교통정리 없는" in q:
        if category == "law":
            weight *= 2.00
        elif category == "easylaw_traffic":
            weight *= 1.80
    ...
    # 고속도로 1차로/앞지르기는 법령과 생활법령 교통운전 우선
    if "고속도로" in q and ("1차로" in q or "추월차로" in q or "앞지르기" in q):
        if category == "law":
            weight *= 1.80
        elif category == "easylaw_traffic":
            weight *= 1.70
    ...
    # 급제동/보복운전은 도로교통법과 생활법령 교통운전 우선
    if "급제동" in q or "급브레이크" in q or "보복" in q or "난폭" in q:
        if category == "law":
            weight *= 1.70
        elif category == "easylaw_traffic":
            weight *= 1.60
...
def retrieve_chunks(query, top_k=4):
    ...
    # 원래 BM25 top을 너무 좁게 보면 boost가 작동할 후보가 없을 수 있으므로 넉넉히 본다.
    for raw_score, idx in scores[:80]:
        item = rows[idx].copy()
        weight = category_weight_for_query(query, item.get("category", ""), item.get("source_file", ""))
        final_score = raw_score * weight

        item["raw_score"] = raw_score
        item["category_weight"] = weight
...
def normalize_issue(text):
    text = text.strip()
    text = re.sub(r"\s+", " ", text)
    text = text.replace("해야 한다", "해야 합니다")
    text = text.replace("확인한다", "확인해야 합니다")
    return text


def is_similar_issue(a, b):
...
def append_unique_issue(issues, item):
    item = normalize_issue(item)

    if not item:
        return

    for old in issues:
        if item == old or is_similar_issue(item, old):
            return
...
def build_issues(question, rule_cards):
    ...
    if ("전동킥보드" in question or "킥보드" in question or "PM" in question) and "횡단보도" in question:
        forced = [
            ...
            "자동차가 횡단보도 주변에서 전방주시와 감속 의무를 다했는지 확인해야 합니다",
        ]
        for x in forced:
            append_unique_issue(issues, x)

    if "자전거횡단도" in question:
        forced = [
            "자동차가 자전거횡단도 앞에서 일시정지했는지 확인해야 합니다",
            "자전거등의 횡단을 방해하거나 위험하게 했는지가 핵심입니다",
            "자전거의 신호위반, 갑작스러운 진입, 야간 시야장애 여부를 함께 확인해야 합니다",
        ]
        for x in forced:
            append_unique_issue(issues, x)
    ...
    if "고속도로" in question and "1차로" in question:
        forced = [
            ...
            "뒤차의 급제동에 정당한 사유가 있었는지와 차간거리·속도를 확인해야 합니다",
        ]
        for x in forced:
            append_unique_issue(issues, x)

    # Rule Card answer_hint는 보조로 넣는다.
    for card in rule_cards:
        hint = card.get("answer_hint", "")
        parts = [x.strip() for x in re.split(r"[.。]\s*", hint) if x.strip()]
        for part in parts:
            append_unique_issue(issues, part)
...
def main():
    ...
    for i, ch in enumerate(chunks, 1):
        raw = ch.get("raw_score", ch["score"])
        weight = ch.get("category_weight", 1.0)
        print(
            f"{i}. score={ch['score']:.3f} "
            f"(raw={raw:.3f}, w={weight:.2f}) | "
            f"{ch['chunk_id']} | {ch['category']} | {ch['source_file']}"
        )
