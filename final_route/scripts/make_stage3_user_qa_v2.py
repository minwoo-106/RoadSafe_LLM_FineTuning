...
OUT_JSONL = OUT_DIR / "roadsafe_stage3_user_qa_v2.jsonl"
OUT_REPORT = OUT_DIR / "roadsafe_stage3_user_qa_v2_report.json"
...
def normalize(s: str) -> str:
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    replace_map = {
        "CCTV footage": "CCTV 영상",
        "Accident report": "사고 접수 자료",
        "Police report": "경찰 신고 또는 사고 접수 자료",
        "Police investigation": "경찰 조사 자료",
        "Witness statements": "목격자 진술",
        "Call the police immediately.": "필요하면 즉시 112에 신고합니다.",
        "911": "112 또는 119",
        "[핵심 판단요소]": "[핵심 판단 요소]",
        "[참고 과실 쌍점]": "[참고 과실 쟁점]",
        "[참고 과실 숾절]": "[참고 과실 쟁점]",
...
def make_user_variants(accident: str):
    ...
    return [
        ...
        f"{accident}. 내가 지금 확인해야 할 자료랑 주의할 점 알려줘.",
        f"{accident}. 이 상황에서 단정하지 말고 필요한 확인사항 위주로 설명해줘.",
    ]
...
def infer_subjects(accident: str):
    ...
    if any(k in accident for k in ["자동차", "차량", "앞차", "뒤차", "내 차", "승용차", "차"]):
        car = "관련 있음"
...
def fallback_factors(accident: str):
    base = [
        "- 사고 위치와 도로 구조",
        "- 각 교통주체의 진행 방향",
        "- 신호와 속도",
    ...
    if "빙판" in accident or "눈" in accident:
        base.extend([
            "- 노면 결빙 여부",
            "- 감속 운전 여부",
            "- 앞차와의 안전거리",
        ])
    if "급제동" in accident or "급브레이크" in accident:
        base.extend([
            "- 급제동의 이유",
            "- 안전거리 확보 여부",
            "- 보복운전 또는 고의성 의심 정황",
    ...
    if "자전거" in accident or "전동킥보드" in accident:
        base.extend([
            "- 통행 위치가 차도, 자전거도로, 횡단보도, 자전거횡단도 중 어디인지",
            "- 탑승 상태인지 내려서 끌고 있었는지",
            "- 등화장치와 안전장비 여부",
        ])
...
def build_answer(accident: str, stage2_output: str) -> str:
    ...
    accident_type = normalize(section(stage2_output, "사고 유형"))
    issues = normalize(section(stage2_output, "참고 과실 쟁점"))
    evidence = normalize(section(stage2_output, "추가 확인 자료"))
    actions = normalize(section(stage2_output, "대처 방법"))
    ...
    factors = fallback_factors(accident)
...
- 정확한 법령 조항, 판례, 과실비율 기준은 RAG 검색 결과와 함께 최종 답변에서 보강해야 합니다.
...
<END>"""
    ...
    for bad in BAD_TEXT:
        if bad in answer:
            raise ValueError(f"bad text remained: {bad}")
...
def main():
    ...
    for src in src_rows:
        ...
        for user_input in make_user_variants(accident):
            ...
            row = {
                "id": f"stage3_user_qa_v2_{len(out_rows)+1:05d}",
    ...
    report = {
        ...
        "note": "Golden eval set is not used for training."
    }
