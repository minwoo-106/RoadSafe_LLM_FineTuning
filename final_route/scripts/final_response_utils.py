...
def build_guardrail_fallback_response(user_input: str, reason: str) -> str:
    ...
    if reason == "rear_end":
        return (
            "[1. 사고 유형]\n"
            "- 사고 유형: 차 대 차 / 정차 또는 감속 중 후방추돌 사고\n"
            "- 도로 상황: 신호 대기, 정차, 감속 또는 정체 구간에서 뒤차가 후방 충돌한 상황\n"
            "- 사용자 진행: 신호 대기 또는 정차 중이었다고 주장\n"
            "- 상대 진행: 뒤차가 후방에서 충돌\n"
            "- 현재 분쟁 포인트: 상대가 사용자의 급정지 또는 갑작스러운 정차를 주장함\n\n"

            "[2. 사고 상황 정리]\n"
            "사용자는 신호 대기 중이었고, 뒤차가 후방에서 사용자 차량을 들이받았다고 설명했습니다. "
            "상대방은 사용자가 갑자기 멈췄다고 주장하고 있으므로, 핵심은 사용자 차량이 정상적인 신호 대기 또는 정차 상태였는지, "
            "뒤차가 충분한 안전거리와 전방주시 의무를 지켰는지입니다.\n\n"

            "[3. 법률·과실 쟁점]\n"
            "- 핵심 쟁점: 사용자가 정상적인 신호 대기 또는 정차 중이었는지, 뒤차가 안전거리 확보와 전방주시를 했는지가 중요합니다.\n"
            "- 사용자에게 유리한 포인트: 신호 대기 중 정차 상태였거나 정상적인 감속 과정이 블랙박스에 확인되면 사용자에게 유리하게 검토될 수 있습니다. 후방 충돌 부위가 명확하면 뒤차의 안전거리 미확보 쟁점이 커집니다.\n"
            "- 사용자에게 불리한 포인트: 불필요한 급정지, 이유 없는 차로 내 정차, 후방 차량이 예상하기 어려운 정차였다는 정황이 있으면 일부 과실 쟁점이 될 수 있습니다.\n\n"

            "[4. 보험사에 물어볼 질문]\n"
            "1. 이번 사고를 후방추돌 사고로 분류했나요?\n"
            "2. 제 차량이 신호 대기 또는 정차 중이었다는 점이 반영됐나요?\n"
            "3. 상대 차량의 안전거리 미확보와 전방주시 여부는 어떻게 검토됐나요?\n"
            "4. 상대가 주장하는 급정지의 근거는 무엇인가요?\n"
            "5. 블랙박스의 어느 장면을 기준으로 과실을 판단했나요?\n"
            "6. 후방 충돌 부위와 양 차량 파손 형태가 과실 판단에 어떻게 반영됐나요?\n\n"

            "[5. 추가로 확보할 자료]\n"
            "- 전방·후방 블랙박스 원본 영상\n"
            "- 신호 대기 또는 정차 상태가 확인되는 장면\n"
            "- 후방 충돌 부위 사진\n"
            "- 상대 차량 전면 파손 사진\n"
            "- 사고 지점 신호등, 차선, 정지선 사진\n"
            "- 상대방의 급정지 주장 내용 기록\n"
            ...
            "[6. 다음 행동 체크리스트]\n"
            "1. 전후방 블랙박스 원본을 백업합니다.\n"
            "2. 정차 시점, 충돌 시점, 신호 상태가 보이는 장면을 캡처합니다.\n"
            "3. 후방 충돌 부위와 상대 차량 전면 파손 부위를 사진으로 남깁니다.\n"
            "4. 보험사에 후방추돌 사고 기준으로 과실 산정 근거를 요청합니다.\n"
            "5. 상대의 급정지 주장에 대해서는 영상상 정차 과정과 신호 상태를 기준으로 대응합니다.\n"
            "6. 상대가 현장 합의나 과실 인정을 요구하면 즉답하지 말고 보험사와 자료 기준으로 대응합니다.\n"
            "<END>"
        )
...
def build_semantic_fallback_response(user_input: str = "", mismatches: List[str] = None) -> str:
    ...
    is_lane_change_contact = (
        any(k in q for k in ["끼어들", "차선 변경", "차로 변경", "진로 변경", "옆 차", "옆차"])
        and any(k in q for k in ["긁", "접촉", "부딪", "사고"])
    )

    if is_lane_change_contact:
        return (
            "[1. 사고 유형]\n"
            "- 사고 유형: 차 대 차 / 차선 변경 또는 진로 변경 중 측면 접촉 사고\n"
            "- 도로 상황: 동일 방향 진행 중 차로 변경 또는 끼어들기 상황\n"
            "- 사용자 진행: 기존 차로에서 직진 중이었다고 주장\n"
            "- 상대 진행: 옆 차가 방향지시등 없이 끼어들거나 진로 변경했다고 주장\n"
            "- 현재 분쟁 포인트: 상대는 사용자가 양보하지 않았다고 주장하는 상황\n\n"

            "[2. 사고 상황 정리]\n"
            "사용자는 2차로에서 직진 중이었고, 옆 차량이 방향지시등 없이 끼어들다가 사용자 차량 측면을 긁었다고 설명했습니다. "
            "이 사고는 보행자 사고가 아니라 차 대 차 진로 변경 또는 차선 변경 중 측면 접촉 사고로 보는 것이 적절합니다. "
            "핵심은 상대 차량의 진로 변경 시점, 방향지시등 사용 여부, 사용자 차량의 기존 차로 정상 진행 여부입니다.\n\n"

            "[3. 법률·과실 쟁점]\n"
            "- 핵심 쟁점: 상대 차량이 차선 변경 또는 진로 변경을 하면서 충분한 안전거리와 방향지시등 사용, 주변 확인 의무를 지켰는지가 중요합니다.\n"
            "- 사용자에게 유리한 포인트: 사용자가 기존 차로에서 직진 중이었고 상대 차량이 갑자기 끼어든 장면이 블랙박스에 확인된다면 유리하게 검토될 수 있습니다. 방향지시등 미사용이나 급진입도 중요한 쟁점입니다.\n"
            "- 사용자에게 불리한 포인트: 사용자가 속도를 높였거나 충분히 회피할 수 있었는데도 감속하지 않았다고 판단되면 일부 과실이 검토될 수 있습니다. 충돌 위치가 상대 차량이 이미 상당 부분 진입한 뒤인지도 확인해야 합니다.\n\n"

            "[4. 보험사에 물어볼 질문]\n"
            "1. 이번 사고를 차선 변경 또는 진로 변경 사고로 분류했나요?\n"
            "2. 상대 차량의 방향지시등 사용 여부가 과실 판단에 반영됐나요?\n"
            "3. 블랙박스상 상대 차량이 언제부터 제 차로로 진입한 것으로 판단했나요?\n"
            "4. 제 차량이 기존 차로에서 정상 직진 중이었다는 점이 반영됐나요?\n"
            "5. 충돌 부위와 접촉 각도를 기준으로 어느 차량이 먼저 진로를 변경한 것으로 보나요?\n"
            "6. 추가 자료를 제출하면 과실 판단 재검토가 가능한가요?\n\n"

            "[5. 추가로 확보할 자료]\n"
            "- 블랙박스 원본 영상\n"
            "- 상대 차량의 방향지시등 사용 여부가 보이는 장면\n"
            "- 양 차량의 차로 위치와 진로 변경 시점이 보이는 장면\n"
            "- 차량 측면 긁힘 부위 사진\n"
            "- 차선, 노면표시, 도로 폭, 주변 차량 흐름이 보이는 현장 사진\n"
            "- 보험사 접수번호와 상대방 주장 내용 기록\n\n"

            "[6. 다음 행동 체크리스트]\n"
            "1. 블랙박스 원본을 백업합니다.\n"
            "2. 상대 차량이 진로 변경을 시작한 시점과 접촉 시점을 캡처합니다.\n"
            "3. 방향지시등 사용 여부와 충돌 부위를 사진으로 정리합니다.\n"
            "4. 보험사에 차선 변경 사고 기준으로 과실 산정 근거를 요청합니다.\n"
            "5. 상대방의 '양보하지 않았다'는 주장에 대해서는 객관 영상과 충돌 위치 중심으로 대응합니다.\n"
            "<END>"
        )
